...
import psycopg2
# Establish variables for database connection setting for ease in changing
# as needed
DATABASE = "postgres"
USERNAME = "vagrant"
HOST = "localhost"
PASSWORD = "vagrant"
# Establish the parameters for connection to the database
conn = psycopg2.connect("dbname=%s user=%s host=%s password=%s" %
                        (DATABASE, USERNAME, HOST, PASSWORD))
...
if choice == 1:
    cur.execute("SELECT articles.title, COUNT(log.path)as num "
                "FROM log JOIN articles ON log.path = '/article/' "
